...
class Panel(ScreenPanel, MmuMixin):
    ...
    def activate(self):
        self.spoolman_start_polling(callback=self.refresh, interval=10)
        ...
        for i in range(num_gates):
            ...
            if str(gate_spool_id[i]) in self.spools:
                ...
                initial_weight = (
                    ...
                    if hasattr(spool, "initial_weight")
                    else spool.filament.weight
                    if hasattr(spool.filament, "weight")
                    else 0
                )
                remaining_weight = (
                    spool.remaining_weight if hasattr(spool, "remaining_weight") else 0
                )
                pixbuf = spool.icon

                remaining_percentage_val = (
                    100 / (initial_weight / remaining_weight) if remaining_weight else 0
                )
                ...
                color_hex = (
                    spool.filament.color_hex[:6].lower()
                    if hasattr(spool.filament, "color_hex")
                    else ""
                )
                color = Gdk.RGBA()
        ...
        if action == "notify_status_update":
            if "mmu" in data:
                ...
                if (
                    "ttg_map" in e_data
                    or "gate" in e_data
                    or "gate_status" in e_data in e_data
