...
class Panel(ScreenPanel, MmuMixin):
    def __init__(self, screen, title):
        ...
        self.ui_sel_tool = self.ui_sel_gate = self.ui_sel_loaded = NOT_SET
    ...
    def init_toolgate_values(self):
        mmu = self._printer.get_stat("mmu")
        if self.ui_sel_tool == NOT_SET:
            self.ui_sel_tool = mmu["tool"]
        if self.ui_sel_gate == NOT_SET:
            self.ui_sel_gate = mmu["gate"]
        if self.ui_sel_loaded == NOT_SET:
    ...
    def get_possible_gates(self, tool):
        ...
        for i in range(num_gates):
            ...
            if endless_spool_groups[check] == group:
                ...
                if best_gate == -1 and gate_status[check] != GATE_EMPTY:
                    best_gate = check
        if best_gate == -1:
            best_gate = gate
    ...
    def select_toolgate(self, widget, toolgate, param=0):
        ...
        if toolgate == "tool":
            ...
            if self.ui_sel_tool == TOOL_GATE_BYPASS:
                ...
                if (
                    self.ui_sel_gate == TOOL_GATE_UNKNOWN
                    or self.ui_sel_gate == TOOL_GATE_BYPASS
                    or self.ui_sel_gate not in possible_gates
                ):
                    self.ui_sel_gate = suggested_gate

        elif toolgate == "gate":
            if param < 0 and self.ui_sel_gate > (
                self.min_tool if self.ui_sel_tool == TOOL_GATE_BYPASS else 0
            ):
                self.ui_sel_gate -= 1
    ...
    def update_toolgate_buttons(self, tool_sensitive=True):
        ...
        if (
            ...
            or self.ui_sel_gate == NOT_SET
            or self.ui_sel_loaded == NOT_SET
            or self.ui_sel_tool == TOOL_GATE_UNKNOWN
            or self.ui_sel_gate == TOOL_GATE_UNKNOWN
            or not tool_sensitive
        ):
    ...
    def update_state_labels(self):
        ...
        tool_str = (
            (f"T{tool}") if tool >= 0 else "Bypass" if tool == TOOL_GATE_BYPASS else "Unknown"
        )
        gate_str = (
            (f"#{gate}") if gate >= 0 else "Bypass" if gate == TOOL_GATE_BYPASS else "Unknown"
        )
