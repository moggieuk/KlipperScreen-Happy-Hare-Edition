...
class Panel(ScreenPanel, MmuMixin):
    def __init__(self, screen, title):
        ...
        for i in range(num_gates):
            ...
            gate_icon = self.labels[f"gate_icon_{i}"] = self._gtk.Image(
                "mmu_gate", width=img_width, height=img_height
            )
        ...
        self.labels["unknown_icon"] = self._gtk.Image(
            "mmu_unknown", width=img_width, height=img_height
        ).get_pixbuf()
        self.labels["available_icon"] = self._gtk.Image(
            "mmu_tick", width=img_width, height=img_height
        ).get_pixbuf()
        self.labels["empty_icon"] = self._gtk.Image(
            "mmu_cross", width=img_width, height=img_height
        ).get_pixbuf()
        ...
        self.labels.update(
            {
                ...
                "c_picker": self._gtk.Button(
                    "mmu_color_chooser", None, "color2", scale=self.bts * 0.8
                ),
    ...
    def select_edit(self, widget, sel_gate):
        ...
        self.labels["filament"].set_active(
            self.ui_gate_status in (GATE_AVAILABLE, GATE_AVAILABLE_FROM_BUFFER)
        )
        ...
        try:
            ...
            if self.can_use_spoolman():
                ...
                if sp is not None:
                    ...
                    self.ui_gate_material = (
                        sp.filament.material if sp.filament.material is not None else ""
                    )
                    self.ui_gate_color = (
                        sp.filament.color_hex[:6].lower()
                        if hasattr(sp.filament, "color_hex")
                        else ""
                    )
    ...
    def select_save(self, widget):
        ...
        self._screen._ws.api.gcode_script(
            f"MMU_GATE_MAP GATE={self.ui_sel_gate} COLOR={self.ui_gate_color} MATERIAL={self.ui_gate_material} AVAILABLE={self.ui_gate_status} SPOOLID={self.ui_gate_spool_id} QUIET=1"
        )
        self.labels["layers"].set_current_page(0)  # Gate list layer
