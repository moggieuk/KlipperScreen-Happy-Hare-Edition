...
class MmuSpoolTray(Gtk.DrawingArea):
    ...
    def _draw(self, widget, cr):
        ...
        # To make rendering very efficient on rpi, cache the scrollable content
        # and reuse it until the underlying spool data or layout changes.
        # Scrolling only repositions the cached surface and rebuilds hitboxes;
        # it does not require the content to be redrawn.
        # Caching can be disabled by setting:
        #   self._enable_render_cache = False
        # Note: this key must include every value that affects the rendered
        #       appearance. If it gets out of sync with update_status() or other
        #       state changes, the cached image can become stale.
        key = (
            width,
            height,
            tuple(
        ...
        content_surface_w = max(1, int(math.ceil(content_w + margin * 2)))

        use_cache = self._enable_render_cache

        if use_cache and self._render_cache is not None and self._render_cache_key == key:
            surface = self._render_cache
        else:
            surface = cairo.ImageSurface(cairo.FORMAT_ARGB32, content_surface_w, height)
            cache_cr = cairo.Context(surface)

            spool_start_x = margin + scroll_pad
            ...
            cache_cr.save()
            cache_cr.rectangle(0, 0, content_surface_w, height)
            cache_cr.clip()
            ...
            for group in groups:
                ...
                for item in group:
                    ...
                    positioned_items.append((item, cx, item_spool_cy))
                ...
                for item, cx, item_spool_cy in reversed(positioned_items):
                    self._draw_spool(
                        cache_cr,
                        cx,
                        item_spool_cy,
        ...
        # Rebuild hitboxes every draw because they depend on scroll position
        self._hitboxes.clear()

        x = margin + scroll_pad
        for group in groups:
            for item in group:
                hitbox_x = x - self._scroll_x
                hitbox_y = top_margin
                hitbox_w = slot_w
        ...
        viewport_surface = self._viewport_surface
        viewport_cr = cairo.Context(viewport_surface)

        # Clear previous frame
        viewport_cr.set_operator(cairo.OPERATOR_CLEAR)
        viewport_cr.paint()
        viewport_cr.set_operator(cairo.OPERATOR_OVER)

        viewport_cr.save()
        viewport_cr.rectangle(0, 0, width, height)
        viewport_cr.clip()

        viewport_cr.set_source_surface(surface, -self._scroll_x, 0)
        viewport_cr.paint()
        ...
        if self._scroll_x > 0:
            ...
            viewport_cr.save()
            viewport_cr.rectangle(0, top_margin, fade_w, height - top_margin - bottom_margin)
            viewport_cr.clip()
        ...
        cr.clip()
        cr.set_source_surface(viewport_surface, 0, 0)
        cr.paint()
        cr.restore()
