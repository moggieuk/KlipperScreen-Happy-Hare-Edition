...
FILAMENT_POS_HOMED_GATE = (
    1  # Homed at either gate or mmu exit sensor (currently assumed mutually exclusive sensors)
)
...
class MmuMixin:
    ...
    def get_tools_for_gate(self, gate):
        ...
        return [tool for tool, mapped_gate in enumerate(ttg_map) if mapped_gate == gate]
    ...
    def get_endless_spool_group_order(self, gate):
        ...
        gates = [g for g, gate_group in enumerate(groups) if gate_group == group]
    ...
    def can_use_spoolman(self):
        return self._printer.spoolman and self._config.get_main_config().getboolean(
            "mmu_use_spoolman", False
        )
    ...
    def spoolman_start_polling(self, callback=None, interval=10):
        ...
        if interval is not None:
            self._spoolman_refresh_timer_id = GLib.timeout_add_seconds(
                interval, self.spoolman_load_spools
            )
        else:
    ...
    def spoolman_load_spools(self):
        self._screen.spoolman_api.load_all_spools(
            allow_archived=False, callback=self._spoolman_load_spools_cb
        )
        return True
...
class MmuUtils:
    ...
    def filament_text_color(filament_color):
        ...
        perceived_lightness = r * 0.2126 + g * 0.7152 + b * 0.0722
